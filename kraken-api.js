...
class KrakenAPI {
    ...
    /**
     * Test API credentials and validate permissions
     */
    async testCredentials(apiKey, apiSecret) {
        try {
            ...
            this.debugLog('Testing API credentials and permissions...', 'api');
            ...
            if (data.success) {
                ...
                if (data.permissions) {
                    const hasTradePermission = data.permissions.includes('Trade') || data.permissions.includes('trade');
                    if (!hasTradePermission) {
                        this.debugLog('⚠️ API key lacks trading permissions - only View access detected', 'warning');
                        return {
                            success: false,
                            error: 'API key lacks trading permissions. Please ensure your Kraken API key has "Trade" permissions enabled.',
                            permissions: data.permissions
                        };
                    }
                    this.debugLog(`✅ API credentials validated with permissions: ${data.permissions.join(', ')}`, 'success');
                } else {
