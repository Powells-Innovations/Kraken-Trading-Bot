...
class TradingApp {
    ...
    updateStatistics() {
        ...
        let displayBalance = stats.accountBalance;
        let balanceLabel = 'Live Account';
        
        // Always use real data - no demo calculations
        document.getElementById('accountBalance').textContent = `£${displayBalance.toFixed(2)}`;
        ...
        document.getElementById('todayPnL').textContent = `£${stats.todayPnL.toFixed(2)}`;
        document.getElementById('totalTrades').textContent = stats.totalTrades;
        document.getElementById('winRate').textContent = `${stats.winRate}%`;
        
        // Debug log the balance update
        this.debugLog(`💳 UI Balance Update: £${displayBalance.toFixed(2)} (${balanceLabel})`, 'info');
    }
     ...
     */
    initializeChart() {
        this.chart = null; // Lightweight Charts doesn't need to keep a reference
        this.updateChart();
    }

    /**
     * Update chart with new data using Lightweight Charts
     */
    updateChart() {
        ...
        if (this.chartInstance) {
            this.chartZoomState = this.chartInstance.timeScale().getVisibleRange();
            this.debugLog(`Preserving zoom state: ${JSON.stringify(this.chartZoomState)}`, 'info');
        }
        
        chartDiv.innerHTML = '';
        if (chartDiv.offsetHeight < 100) chartDiv.style.height = '400px';
        if (chartDiv.offsetWidth < 100) chartDiv.style.width = '100%';
        chartDiv.style.background = '#222';
        this.debugLog(`Chart container size after adjustment: ${chartDiv.offsetWidth}x${chartDiv.offsetHeight}`, 'info');
        
        const chartData = this.tradingBot.getChartData(this.selectedPair, this.chartType);
        if (!chartData || chartData.length === 0) {
            chartDiv.innerHTML = '<div style="color:#ccc;text-align:center;padding:2em;">No chart data available for this pair.</div>';
            return;
        }
        
        // Filter data by time range
        const filteredData = this.filterDataByTimeRange(chartData);
        this.debugLog(`Time range ${this.timeRange}: ${filteredData.length} data points from ${chartData.length} total`, 'info');
        
        // Limit data points to last 100 for better zoom
        const limitedData = filteredData.slice(-100);
        this.debugLog(`Chart data points: ${limitedData.length} (limited from ${filteredData.length})`, 'info');
        
        // Check for different possible global names for LightweightCharts
        const LightweightCharts = window.LightweightCharts || window.lightweightCharts;
        if (LightweightCharts && LightweightCharts.version) {
            this.debugLog(`LightweightCharts version: ${LightweightCharts.version}`, 'info');
        }
        if (!LightweightCharts || typeof LightweightCharts.createChart !== 'function') {
            this.debugLog('LightweightCharts library is not loaded or createChart is not a function', 'error');
            this.debugLog('Available globals: ' + Object.keys(window).filter(k => k.toLowerCase().includes('chart') || k.toLowerCase().includes('lightweight')).join(', '), 'error');
            this.createFallbackChart(chartDiv, limitedData);
            return;
        }
        
        try {
            this.debugLog('LightweightCharts library loaded successfully', 'success');
            const chart = LightweightCharts.createChart(chartDiv, {
                width: chartDiv.offsetWidth || 600,
                height: chartDiv.offsetHeight || 400,
                layout: { background: { color: '#181c2a' }, textColor: '#d1d4dc' },
                grid: { vertLines: { color: '#222' }, horzLines: { color: '#222' } },
                timeScale: { 
                    timeVisible: true, 
                    secondsVisible: false,
                    rightOffset: 5,
                    barSpacing: 8, // Increased from 3 for better zoom
                    minBarSpacing: 4, // Increased from 2
                    fixLeftEdge: true,
                    fixRightEdge: true,
                    tickMarkFormatter: (timestamp) => {
                        // Convert UNIX timestamp (seconds) to local time string (GMT/BST)
            ...
            // Store chart instance for zoom state management
            this.chartInstance = chart;
            
            if (this.chartType === 'candlestick' && typeof chart.addCandlestickSeries === 'function') {
                try {
                    this.debugLog('Creating candlestick chart...', 'info');
                    ...
                    // Validate and format candlestick data
                    const formattedData = limitedData
                        .filter(d => d.open && d.high && d.low && d.close && d.time)
                        .map(d => {
                    ...
                    if (formattedData.length === 0) {
                        this.debugLog('No valid candlestick data available', 'warning');
                        this.createFallbackChart(chartDiv, limitedData);
                        return;
                    }
                    
                    this.debugLog(`Candlestick data sample: ${JSON.stringify(formattedData[0])}`, 'info');
                    candleSeries.setData(formattedData);
                    this.debugLog(`Candlestick chart created with ${formattedData.length} data points`, 'success');
                } catch (error) {
                    this.debugLog(`Candlestick series error: ${error.message}`, 'error');
                    this.createFallbackChart(chartDiv, limitedData);
                }
            } else if (typeof chart.addLineSeries === 'function') {
                try {
                    this.debugLog('Creating line chart...', 'info');
                    const lineSeries = chart.addLineSeries({
                        color: '#4CAF50',
                        lineWidth: 2
                    });
                    
                    const formattedData = limitedData.map(d => ({
                        time: Math.floor(d.time / 1000),
                        value: d.close || d.price
                    }));
                    
                    lineSeries.setData(formattedData);
                    this.debugLog(`Line chart created with ${formattedData.length} data points`, 'success');
                } catch (error) {
                    this.debugLog(`Line series error: ${error.message}`, 'error');
                    this.createFallbackChart(chartDiv, limitedData);
                }
            } else {
                this.debugLog('No chart series methods available', 'error');
                this.createFallbackChart(chartDiv, limitedData);
            }
            
            // Restore zoom state if available
            if (this.chartZoomState) {
                setTimeout(() => {
                    try {
                        chart.timeScale().setVisibleRange(this.chartZoomState);
                        this.debugLog('Zoom state restored', 'info');
                    } catch (error) {
                        this.debugLog(`Failed to restore zoom state: ${error.message}`, 'warning');
                    }
                }, 100);
            }
            
        } catch (error) {
            this.debugLog(`Chart creation error: ${error.message}`, 'error');
            this.createFallbackChart(chartDiv, limitedData);
        }
    }
     ...
     */
    filterDataByTimeRange(chartData) {
        ...
        switch (this.timeRange) {
            ...
            case '1w':
                ...
                break;
            default:
                timeLimit = now - (15 * 60 * 1000); // Default to 15m
        }
        
        return chartData.filter(point => point.time >= timeLimit);
    }

    /**
     * Create a fallback chart when LightweightCharts fails
     */
    createFallbackChart(chartDiv, chartData) {
        const lastData = chartData.length > 0 ? chartData[chartData.length - 1] : {};
        const currentPrice = typeof lastData.close === 'number' ? lastData.close : undefined;
        const priceDisplay = typeof currentPrice === 'number' ? currentPrice.toFixed(4) : 'N/A';
        const symbolName = this.marketType === 'crypto' ? 
            this.krakenAPI.pairNames[this.selectedPair] : 
            this.stockAPI.stockNames[this.selectedPair];
        const currency = this.marketType === 'crypto' ? '£' : '$';
        
        chartDiv.innerHTML = `
            <div style="color:#f66;text-align:center;padding:1em;background:#181c2a;border-radius:8px;">
                <div style="font-size:14px;margin-bottom:10px;">📈 Price Chart (Fallback Mode)</div>
                <div style="font-size:12px;color:#888;">Chart library error. Using price display only.</div>
                <div id="simplePriceDisplay" style="font-size:24px;font-weight:bold;color:#4CAF50;margin-top:10px;">${currency}${priceDisplay}</div>
                <div style="font-size:12px;color:#888;margin-top:5px;">${symbolName || this.selectedPair}</div>
            </div>
        `;
    }
    ...
    selectPair(symbol) {
        ...
        // Find the button for this symbol and activate it
        buttons.forEach(btn => {
            if (btn.textContent.includes(symbol) || btn.textContent === symbol) {
    ...
    setChartType(type) {
        ...
        buttons.forEach(btn => btn.classList.remove('active'));
        // Find the button for this type and activate it
        buttons.forEach(btn => {
    ...
    startLatencyTester() {
        ...
        const pingFinnhub = async () => {
            const start = performance.now();
            try {
                // Fetch real market data from Finnhub
                const url = `https://finnhub.io/api/v1/quote?symbol=AAPL&token=${this.stockAPI.apiKey}`;
                ...
                const latency = Math.round(performance.now() - start);
                latencyEl.textContent = `${latency} ms`;
                this.debugLog(`🌐 Finnhub API latency: ${latency}ms`, 'info');
            } catch (error) {
                latencyEl.textContent = '-- ms';
                this.debugLog(`❌ Finnhub API latency test failed: ${error.message}`, 'error');
            }
        };
        ...
        ping();
        this.latencyInterval = setInterval(ping, 5000);
        this.debugLog(`🔄 Latency tester started for ${this.marketType} market`, 'info');
    }

    /**
     * Set time range for chart data
     */
    setTimeRange(range) {
