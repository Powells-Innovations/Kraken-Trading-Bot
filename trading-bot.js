...
class TradingBot {
    ...
    async swingTrade(tickerData) {
        ...
        let best = null;
        for (const r of results) {
            if (!r.decision.shouldTrade || !this.canTrade(r.pair)) continue;
            if (!best || r.decision.confidence > best.decision.confidence) {
                best = r;
            }
        }
        // Only trade the best opportunity (if any)
        if (best && this.tradeCounter < 3) {
            if (this.executeTrade(best.pair, best.decision.side, best.data.price, `AI Swing: ${best.decision.reason}`, best.decision.stopLoss, best.decision.takeProfit)) {
                this.tradeCounter++;
                this.debugLog(`🤖 AI Trade: ${best.decision.side} ${best.pair} | Confidence: ${best.decision.confidence}% | R/R: ${best.decision.riskRewardRatio}:1 | SL: £${best.decision.stopLoss} | TP: £${best.decision.takeProfit}`, 'success');
    ...
    async getSwingDecision(pair, data) {
        ...
        return {
            shouldTrade: finalSide !== null && aiLevels !== null,
            side: finalSide,
            reason: reasons.slice(0, 3).join(', '),
            confidence: Math.max(confidence/100, nnResult.confidence),
     ...
     */
    canTrade(pair) {
        // Allow multiple trades per pair and no trade interval cooldown

        // Check max active trades
        if (Object.keys(this.activeTrades).length >= this.settings.maxActiveTrades) {
            this.debugLog(`[canTrade] Blocked: max active trades reached (${this.settings.maxActiveTrades})`, 'warning');
            return false;
        }
